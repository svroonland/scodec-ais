...
class AisCodecSpec
    ...
    with OptionValues
    with EitherValues
    with Inspectors with Inside with AisMessageCodec {
  implicit val charset = java.nio.charset.StandardCharsets.US_ASCII

  "Suffixed decoder" should {
    "decode properly" in {
      val aisMessage = BitVector.encodeString("!AIVDM,2,1,5,A,53KQ<L01pv7ptTtN22059@DlU>2222222222220l1@D555V00:R0CRkp8888,0*41").right.get
    ...
    Codec[NmeaMessage]
      ...
      case Failure(cause) =>
        throw new IllegalArgumentException(
          s"Failed to decode NMEA message ${cause}, ${cause}")
    }
    ...
    "decode more messages" in {
      forAll(messages) { msg =>
        withClue(msg) {
          ...
          // Decode the payload
          println(s"NMEA size: ${msg.lines.mkString("").size}, payload size: ${payload.bytes.size}")
          val message = Codec[AisMessage].decodeValue(payload)
          println(message)
        }
      }
  ...
  val messages = Seq(
    "!AIVDM,1,1,,A,E>k`@f@:aPQRgQT6?b2ab000000<h1SJ9?KO800000lP00,4*4E",
    "!AIVDM,1,1,,A,43vR93iv9t7;b1F?`FRD5wG000S:,0*17",
...
//      |!AIVM,2,2,8,A,221I:PF55v:ok03Sp888888888888888880,2*28""".stripMargin,
    """!AIVDM,2,1,,B,56K2?>@2BD7l`C?3GB1<PtDV0l59F2222222221I:`D6:5tU0@?RT82l,0*2E
